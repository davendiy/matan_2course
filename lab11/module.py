...
def mass_center(a: float, b: float, u: callable, v: callable, r: callable):
    ...
    s = (b-a)*(_max - _min)         # площа квадрата
    start = [a, _min]               # межі
    end = [b, _max]
    print(s)
    if s < 2000:                 # оптимізація
        density = 4              # перевіряємо площу квадрата: в залежності від величини
    else:
        density = 8 - int(log(s, 4))   # вибираємо щільність розбиття і чи потрібно точніше рахувати
    ...
    mass = naive_integral(r, check_set, density, 2, start, end)             # маса
    res1 = naive_integral(func1, check_set, density, 2, start, end) / mass  # перша координата
    res2 = naive_integral(func2, check_set, density, 2, start, end) / mass  # друга
    return res1, res2
...
def naive_integral(func: callable, check_set: callable, n: int, m: int, start: list, end: list):
    """ Кратна інтегральна сума на множині, яку задає функція check_set.
        Зрозумів, що оптимізація занадто багато забирає часу і тому прибрав її.
